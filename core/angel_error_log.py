"""
Angel One error logger — appends API/order failures to a JSON file so they can be
...
ERROR_LOG_PATH = Path(config.LOGS_DIR) / "angel_errors.json"
MAX_ERRORS = 500
...
def log_error(source: str, error: str, symbol: str = "", detail: str = ""):
    ...
    with _lock:
        try:
            ...
            logger.warning("angel_error_log: could not write: %s", e)
