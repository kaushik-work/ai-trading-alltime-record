...
class BacktestEngine:
    ...
    def run(self, symbol: str, period: str = "60d",
        ...
        """
        if strategy == "Musashi":
            return self.run_trend_rider(symbol, period=period, interval=interval,
    ...
    # ── Musashi backtest (NIFTY intraday, 15-min) ─────────────────────────
    ...
    def run_trend_rider(
        ...
        Signal: EMA stack + VWAP bias + EMA21 pullback + HA confirmation
                + RSI filter + volume + swing structure
        Timeframe: 15-minute bars
        R:R: 1:2.5 (SL = 1.25× ATR, TP = 3.125× ATR)
        Max 2 trades/day | Entry window 9:45-11:30 + 13:30-14:30
        """
        import numpy as np
        from strategies.nifty_intraday import (
            score_signal as intraday_score,
            in_entry_window,
            MAX_TRADES_DAY,
            EOD_EXIT,
        )
        ...
        for day_idx, day in enumerate(days):
            ...
            def _close_tr(exit_px: float, reason: str):
                nonlocal equity, day_trade_count, position
                trade_delta  = position.get("option_delta", opt_delta)
                gross        = self._pnl_options(position, exit_px, trade_delta, lot_size)
                chg          = charges_for_trade(
            ...
            for pos, (ts, row) in enumerate(day_df.iterrows()):
                ...
                if position:
                    ...
                    if position["side"] == "BUY":
                        if lo <= position["sl"]:
                            _close_tr(position["sl"], "SL"); continue
                        if hi >= position["tp"]:
                            _close_tr(position["tp"], "TP"); continue
                    else:
                        if hi >= position["sl"]:
                            _close_tr(position["sl"], "SL"); continue
                        if lo <= position["tp"]:
