"""ETH strategy enhanced with supply/demand zone filter."""
import sys
sys.path.insert(0, '.')
import pandas as pd
import numpy as np
...
def compute_supply_demand_zones(df, lookback_minutes, zone_width_pct=0.004):
    r_high = df['high'].rolling(lookback_minutes, min_periods=lookback_minutes).max()
    r_low = df['low'].rolling(lookback_minutes, min_periods=lookback_minutes).min()
    supply_zone_bot = r_high * (1 - zone_width_pct)
    supply_zone_top = r_high * (1 + zone_width_pct)
    demand_zone_top = r_low * (1 + zone_width_pct)
    demand_zone_bot = r_low * (1 - zone_width_pct)
    return supply_zone_bot, supply_zone_top, demand_zone_bot, demand_zone_top


def run_strategy(df, s, eff_lev=15.0, sd_lookback_min=None, sd_filter_mode='skip_supply_demand',
                 use_vol_filter=None):
    ...
    if sd_lookback_min:
        supply_bot, supply_top, demand_bot, demand_top = compute_supply_demand_zones(df, sd_lookback_min)
    else:
        supply_bot = supply_top = demand_bot = demand_top = pd.Series(0, index=df.index)
    ...
    for i in range(start_i, n - 1):
        ...
        if sd_lookback_min:
            in_supply = (ci >= supply_bot.iloc[i]) & (ci <= supply_top.iloc[i])
            in_demand = (ci >= demand_bot.iloc[i]) & (ci <= demand_top.iloc[i])
            if sd_filter_mode == 'skip_supply_demand':
                if long_sig[i] and in_supply:
                    continue
                if short_sig[i] and in_demand:
                    continue
...
if __name__ == '__main__':
    ...
    print('\n=== Skip buying at supply, skip selling at demand (rolling high/low zones) ===')
    for days in [1, 3, 7, 14, 30]:
        cap, dd, trades = run_strategy(df, s, eff_lev=15.0, sd_lookback_min=days*24*60)
        print_result(f'rolling S/D {days}d skip', cap, dd, trades)
    
    print('\n=== Combine 7d S/D skip + vol<33% ===')
    cap, dd, trades = run_strategy(df, s, eff_lev=15.0, sd_lookback_min=7*24*60, use_vol_filter=0.33)
    print_result('7d S/D skip + vol<33%', cap, dd, trades)
